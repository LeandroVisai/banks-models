...
Schema (normalizado):
  ...
  chunks                     → N filas por documento, con embedding vector(768)
...
def detect_embedding_dim() -> int:
    ...
    4. Fallback 1024 (Qwen3-Embedding, compatibilidad con qwen repo)
    """
...
def build_indices_sql() -> list[str]:
    return [
        # documents
        ...
        f"CREATE INDEX IF NOT EXISTS idx_chunks_policy ON {CHUNKS_TABLE}(is_policy_decision) WHERE is_policy_decision = TRUE",
        f"CREATE INDEX IF NOT EXISTS idx_chunks_fwd ON {CHUNKS_TABLE}(is_forward_looking) WHERE is_forward_looking = TRUE",
        # chunks — GIN
        ...
        f"CREATE INDEX IF NOT EXISTS idx_chunks_embedding_hnsw ON {CHUNKS_TABLE} USING hnsw (embedding vector_cosine_ops) WITH (m = 16, ef_construction = 64)",
                ...
                if row and row[0]:
                    ...
                    if m:
                        ...
                        if existing_dim != dim:
                            ...
                            print("[03] Opciones:\n  1) Crear una tabla nueva para este modelo: exporta RAG_TABLE_PREFIX='qwen_' y ejecuta 'setup' antes de 'load'.\n  2) Si quieres sobrescribir la tabla existente, ejecuta 'python 03_database.py reset' para recrearla con la nueva dimensión (destructivo).\n  3) Re-vectoriza con una dimensión compatible (no recomendado si quieres usar Qwen3-Embedding).")
                            return 2
